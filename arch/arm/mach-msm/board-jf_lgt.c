...
{
	pr_info("%s is called\n", __func__);
	gpio_tlmm_config(GPIO_CFG(64, 0, GPIO_CFG_INPUT,
		GPIO_CFG_NO_PULL, GPIO_CFG_2MA), 1);
	gpio_tlmm_config(GPIO_CFG(29, 0, GPIO_CFG_INPUT,
		GPIO_CFG_NO_PULL, GPIO_CFG_2MA), 1);
}
#endif
...
#if defined(CONFIG_TDMB) || defined(CONFIG_TDMB_MODULE)
struct pm8xxx_mpp_config_data tdmb_mpp_low = {
	.type = PM8XXX_MPP_TYPE_D_OUTPUT,
	.level = PM8921_MPP_DIG_LEVEL_S4,
	.control = PM8XXX_MPP_DOUT_CTRL_LOW,
};
struct pm8xxx_mpp_config_data tdmb_mpp_high = {
	.type = PM8XXX_MPP_TYPE_D_OUTPUT,
	.level = PM8921_MPP_DIG_LEVEL_S4,
	.control = PM8XXX_MPP_DOUT_CTRL_HIGH,
};
struct pm_gpio tdmb_int_cfg = {
	.direction = PM_GPIO_DIR_IN,
	.pull = PM_GPIO_PULL_NO,
	.vin_sel = PM_GPIO_VIN_S4,
	.function = PM_GPIO_FUNC_NORMAL,
...
struct pm8xxx_mpp_config_data tdmb_mpp_irq = {
...
{
	if (!gpio_request(GPIO_TDMB_SPI_MOSI, "tdmb_spi_mosi")) {
		gpio_tlmm_config(GPIO_CFG(GPIO_TDMB_SPI_MOSI, GPIOMUX_FUNC_GPIO,
			GPIO_CFG_INPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_8MA),
			GPIO_CFG_ENABLE);
	}
	if (!gpio_request(GPIO_TDMB_SPI_MISO, "tdmb_spi_miso")) {
		gpio_tlmm_config(GPIO_CFG(GPIO_TDMB_SPI_MISO, GPIOMUX_FUNC_GPIO,
			GPIO_CFG_INPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_8MA),
			GPIO_CFG_ENABLE);
	}
	if (!gpio_request(GPIO_TDMB_SPI_CS, "tdmb_spi_cs")) {
		gpio_tlmm_config(GPIO_CFG(GPIO_TDMB_SPI_CS, GPIOMUX_FUNC_GPIO,
			GPIO_CFG_INPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_8MA),
			GPIO_CFG_ENABLE);
	}
	if (!gpio_request(GPIO_TDMB_SPI_CLK, "tdmb_spi_clk")) {
		gpio_tlmm_config(GPIO_CFG(GPIO_TDMB_SPI_CLK, GPIOMUX_FUNC_GPIO,
			GPIO_CFG_INPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_8MA),
			GPIO_CFG_ENABLE);
	}
	if (!gpio_request(GPIO_TDMB_EN, "tdmb_en")) {
		pm8xxx_mpp_config(GPIO_TDMB_EN, &tdmb_mpp_low);
	}
	if (!gpio_request(GPIO_TDMB_RST, "tdmb_rst")) {
		pm8xxx_mpp_config(GPIO_TDMB_RST, &tdmb_mpp_low);
	}
}
static void tdmb_gpio_on(void)
{
	...
	gpio_tlmm_config(GPIO_CFG(GPIO_TDMB_SPI_CLK, GPIOMUX_FUNC_1,
		GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_8MA),
		GPIO_CFG_ENABLE);
	pm8xxx_mpp_config(GPIO_TDMB_EN, &tdmb_mpp_high);
	usleep_range(10000, 10000);
	pm8xxx_mpp_config(GPIO_TDMB_RST, &tdmb_mpp_low);
	usleep_range(2000, 2000);
	pm8xxx_mpp_config(GPIO_TDMB_RST, &tdmb_mpp_high);
	usleep_range(10000, 10000);
}
...
{
#if defined(CONFIG_TDMB_ANT_DET)
	pm8xxx_mpp_config(PM8821_MPP_PM_TO_SYS(3), &tdmb_mpp_irq);
	tdmb_pdata.gpio_ant_det = PM8821_MPP_PM_TO_SYS(3);
	tdmb_pdata.irq_ant_det = PM8821_MPP_IRQ(PM8821_IRQ_BASE, 3),
#endif

	pm8xxx_gpio_config(GPIO_TDMB_INT, &tdmb_int_cfg);
	tdmb_pdata.irq = PM8921_GPIO_IRQ(PM8921_IRQ_BASE, 28),
...
static void sec_jack_init(void)
{
	...
	if (system_rev == BOARD_REV00) {
		ret = gpio_request(GPIO_EAR_BIAS_DISCHARGE,
			"ear_bias_discharge");
		if (ret) {
			pr_err("%s : gpio_request failed for %d\n", __func__,
